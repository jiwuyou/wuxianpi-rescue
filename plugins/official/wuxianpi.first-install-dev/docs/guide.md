...
这是用于真机验证下一版正式首次安装链路的开发插件。它会先安装并启动独立的 `wuxianpi.background-run-guide`，等待后台运行设置完成后，再使用当前完整的 `openhouse-core-stack` 正式资源集合继续安装；不再转交正式首次安装插件。
...
镜像阶段通过后，插件继续完成 Android 资源投递、当前正式资源集合补齐、运行激活、Package reconcile、Android 私有连接保存、Ubuntu 安装和安装收尾。验证通过后，开发插件内容可以直接发布为下一版正式首次安装插件。

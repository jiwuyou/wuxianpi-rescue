...
`wuxianpi.first-install-dev 0.2.6` 用当前正式资源集合验证下一版正式首次安装流程。它会先安装并启动 `wuxianpi.background-run-guide`，等待后台运行设置完成后，再将权限准备、公网镜像测速、Android 资源投递、市场差异补齐、运行激活和 Ubuntu 安装拆成独立阶段。技术流程结束后单独启动 `wuxianpi.setup-finish` 收尾插件。后续阶段失败不会删除已经完成的前序阶段。
